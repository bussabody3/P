...
def ranks_reply_promote(c,m,k):
    ...
    if text == 'تفعيل منع التصفيه':
      if not owner_pls(m.from_user.id, m.chat.id):
        return m.reply(f'{k} هذا الامر يخص ( المالك وفوق ) بس')
      else:
        if r.get(f'{m.chat.id}:antiselfkill:{Dev_Zaid}'):
          return m.reply(f'「 {m.from_user.mention} 」\n{k} منع التصفيه مفعل من قبل\n☆')
        else:
          r.set(f'{m.chat.id}:antiselfkill:{Dev_Zaid}', 1)
          return m.reply(f'{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت منع التصفيه\n☆')

    if text == 'تعطيل منع التصفيه':
      if not owner_pls(m.from_user.id, m.chat.id):
        return m.reply(f'{k} هذا الامر يخص ( المالك وفوق ) بس')
      else:
        if not r.get(f'{m.chat.id}:antiselfkill:{Dev_Zaid}'):
          return m.reply(f'「 {m.from_user.mention} 」\n{k} منع التصفيه معطل من قبل\n☆')
        else:
          r.delete(f'{m.chat.id}:antiselfkill:{Dev_Zaid}')
          return m.reply(f'{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت منع التصفيه\n☆')
    ...
    if text == 'رفع مشرف' and m.reply_to_message and m.reply_to_message.from_user:
        ...
        try:
            ...
            return m.reply(f'⇜ تم رفعه مشرف .. بنجاح ✅')
        except ChatAdminRequired:
            return m.reply(f'⇜ خطأ البوت ليس معه صلاحيه الرفع')
        except Exception as e:
            return m.reply(f'⇜ خطأ البوت ليس معه صلاحيه {e}')

    if text.startswith('رفع مشرف ') and not m.reply_to_message:
        if not owner_pls(m.from_user.id, m.chat.id):
            return m.reply(f'{k} هذا الامر يخص ( المالك وفوق ) بس')
        try:
            user = text.split()[2]
            get_user = c.get_users(user)
            c.promote_chat_member(
                chat_id=m.chat.id,
                user_id=get_user.id,
                privileges=ChatPrivileges(
