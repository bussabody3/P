...
async def private_func(c,m,k):
  ...
  if text == '/start' and not dev_pls(m.from_user.id,m.chat.id):
     await c.send_video(
         chat_id=m.chat.id,
         video='https://h.uguu.se/KBifGOpz.mp4',
         caption=f'''
**⇜ اطلق بوت اسمي ❨ {name} ❩ 🧚
⇜ اختصاصي البوت حماية المجموعات 
⇜ العاب ، تسلية ، هواي اوامر 
⇜ فقط ارفعني إشراف كامل الصلاحيات
⇜ وسيتم تفعيل البوت تلقائيا 

⇜ لعرض الكيبورد الاوامر الخدمية إضغط ← /TEP 🖤**''',
...
- في حال انك بالتوب واسمك مزخرف راح يصفيه البوت تلقائي''',reply_markup=InlineKeyboardMarkup ([[InlineKeyboardButton (f"تحديثات {name} 🍻", url=f't.me/{channel}')]]))
  ...
  if text == '/start' and dev_pls(m.from_user.id,m.chat.id):
     reply_markup = ReplyKeyboardMarkup(
      ...
      placeholder='@A7718BOT - @Tepthon 🧚‍♀️'
     )
...
async def run_cmd(c: Client, m: Message):
    if len(m.text.split()) < 2:
        return await m.reply_text("<b>Usage:</b> <code>/cmd [command]</code>")
