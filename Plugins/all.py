...
@Client.on_callback_query(filters.regex("^like:"))
async def like_callback(c, q):
    target_id = int(q.data.split(":")[1])
    user_id = q.from_user.id
    
    if r.sismember(f"{Dev_Zaid}:liked:{target_id}", user_id):
        r.srem(f"{Dev_Zaid}:liked:{target_id}", user_id)
        r.decr(f"{Dev_Zaid}:likes:{target_id}")
        await q.answer("تمت إزالة الإعجاب 💔", show_alert=True)
    else:
        r.sadd(f"{Dev_Zaid}:liked:{target_id}", user_id)
        r.incr(f"{Dev_Zaid}:likes:{target_id}")
        await q.answer("تم تسجيل إعجابك ❤️", show_alert=True)
        
    likes = int(r.get(f"{Dev_Zaid}:likes:{target_id}") or 0)
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"ʟɪᴋᴇ ❤️ ⮕ {likes}", callback_data=f"like:{target_id}"),
         InlineKeyboardButton("ʟɪᴋᴇʀѕ 👣", callback_data=f"likers:{target_id}")]
    ])
    await q.edit_message_reply_markup(reply_markup=keyboard)

@Client.on_callback_query(filters.regex("^likers:"))
async def likers_callback(c, q):
    target_id = int(q.data.split(":")[1])
    if not q.from_user.id == target_id:
        return await q.answer("هذا الزر خاص بصاحب الايدي فقط! ⚠️", show_alert=True)
        
    liked_users = r.smembers(f"{Dev_Zaid}:liked:{target_id}")
    if not liked_users:
        return await q.answer("لا يوجد معجبون بعد ☹️", show_alert=True)
    ...
    for uid in liked_users:
        if count >= 20:
            break
        uid = int(uid)
        name = r.get(f"{uid}:bankName") or str(uid)
        txt += f"- {name}\n"
        count += 1
    await q.answer(txt, show_alert=True)

@Client.on_callback_query(filters.regex("^rate:"))
async def rate_callback(c, q):
    if r.get(f'{Dev_Zaid}:rated:{q.from_user.id}'):
        return await q.answer("لقد قمت بالتقييم مسبقاً!", show_alert=True)
        
    rating = q.data.split(":")[1]
    r.incr(f'{Dev_Zaid}:rating:{rating}')
    r.set(f'{Dev_Zaid}:rated:{q.from_user.id}', 1)
    
    # Update current message with new values
    r1 = int(r.get(f'{Dev_Zaid}:rating:1') or 0)
    r2 = int(r.get(f'{Dev_Zaid}:rating:2') or 0)
    r3 = int(r.get(f'{Dev_Zaid}:rating:3') or 0)
    r4 = int(r.get(f'{Dev_Zaid}:rating:4') or 0)
    r5 = int(r.get(f'{Dev_Zaid}:rating:5') or 0)
    
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"⭐ ⮕ {r1}", callback_data="rate:1")],
        [InlineKeyboardButton(f"⭐⭐ ⮕ {r2}", callback_data="rate:2")],
        [InlineKeyboardButton(f"⭐⭐⭐ ⮕ {r3}", callback_data="rate:3")],
        [InlineKeyboardButton(f"⭐⭐⭐⭐ ⮕ {r4}", callback_data="rate:4")],
        [InlineKeyboardButton(f"⭐⭐⭐⭐⭐ ⮕ {r5}", callback_data="rate:5")]
    ])
    
    await q.answer("تم تسجيل تقييمك بنجاح! شكراً لك.", show_alert=True)
    await q.edit_message_reply_markup(reply_markup=keyboard)
...
async def on_zbi(c: Client, m: Message):
    ...
    if not m.from_user or dev_pls(m.from_user.id, m.chat.id):
        return
   ...
   """
    ...
    if text == "تعطيل اسمي":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableNAME:{Dev_Zaid}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} امر اسمي معطل من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:disableNAME:{Dev_Zaid}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت امر اسمي\n☆"
                )
    ...
    if text == "تفعيل اسمي":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:disableNAME:{Dev_Zaid}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} امر اسمي مفعل من قبل\n☆"
                )
            else:
                r.delete(f"{m.chat.id}:disableNAME:{Dev_Zaid}")
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت امر اسمي\n☆"
                )
    ...
    if text == "تعطيل ايديي":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if r.get(f"{m.chat.id}:disableIDY:{Dev_Zaid}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} امر ايديي معطل من قبل\n☆"
                )
            else:
                r.set(f"{m.chat.id}:disableIDY:{Dev_Zaid}", 1)
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر عطلت امر ايديي\n☆"
                )
    ...
    if text == "تفعيل ايديي":
        if not mod_pls(m.from_user.id, m.chat.id):
            ...
            if not r.get(f"{m.chat.id}:disableIDY:{Dev_Zaid}"):
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} امر ايديي مفعل من قبل\n☆"
                )
            else:
                r.delete(f"{m.chat.id}:disableIDY:{Dev_Zaid}")
                return m.reply(
                    f"{k} من「 {m.from_user.mention} 」\n{k} ابشر فعلت امر ايديي\n☆"
                )
...
☆""",
    ...
    if text == 'تفعيل صورتي':
        if r.get(f'{m.chat.id}:photo:{Dev_Zaid}'):
            return m.reply(f'⇜ صورتي مفعلة من قبل')
        else:
            r.set(f'{m.chat.id}:photo:{Dev_Zaid}', 'enable')
            return m.reply(f'⇜ تم تفعيل صورتي بنجاح')

    if text == 'تعطيل صورتي':
        if not r.get(f'{m.chat.id}:photo:{Dev_Zaid}'):
            return m.reply(f'⇜ صورتي معطلة من قبل')
        else:
            r.delete(f'{m.chat.id}:photo:{Dev_Zaid}')
            return m.reply(f'⇜ تم تعطيل صورتي بنجاح')

    if text in ['صورتي', 'اا']:
        if not r.get(f'{m.chat.id}:photo:{Dev_Zaid}'):
            return m.reply("⇜ عذراً، هذا الأمر معطل في هذه المجموعة")
        photos_count = c.get_chat_photos_count(m.from_user.id)
        if photos_count > 0:
            for photo in c.get_chat_photos(m.from_user.id, limit=1):
                m.reply_photo(photo.file_id, caption=f"⇜ عدد صور حسابك : ❨ {photos_count} ❩")
        else:
            m.reply(f"⇜ ليس لديك صور في حسابك")
    ...
    if (
        ...
        if admin_pls(m.from_user.id, m.chat.id):
            return m.reply(
                """⎙┊يـوجـد ←  𝟏𝟎 قوائـم فـي البــوت 🎖
ٴ⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆
 ⦇ ① ⦈  ←  اوامــر الحماية & القفل
 ⦇ ② ⦈  ←  اوامــر التفعيل و التعطيل
 ⦇ ③ ⦈  ←  قوائــم الإدارة 🎖
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        ...
                        [
                            ...
                            InlineKeyboardButton("𝟱", callback_data=f"commands4:{m.from_user.id}"),
                            InlineKeyboardButton("𝟲", callback_data=f"commands6:{m.from_user.id}"),
                        ],
                        [
                            InlineKeyboardButton("𝟳", callback_data=f"commands9:{m.from_user.id}"),
                            InlineKeyboardButton("𝟖", callback_data=f"commands7:{m.from_user.id}"),
                            InlineKeyboardButton("𝟗", callback_data=f"commands8:{m.from_user.id}"),
                        ],
                        [
                            InlineKeyboardButton("𝟭𝟎", callback_data=f"commands10:{m.from_user.id}"),
                        ]
                    ]
                ),
            )
...
""",
            reply_markup=InlineKeyboardMarkup(
                [
                    ...
                    [
                        InlineKeyboardButton("رجوع 🔙", callback_data=f"commands_back:{m.from_user.id}")
                    ]
                ]
            ),
        )
    ...
    if m.data == f"commands_back:{m.from_user.id}":
        m.edit_message_text(
            """⎙┊يـوجـد ←  𝟏𝟎 قوائـم فـي البــوت 🎖
ٴ⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆
 ⦇ ① ⦈  ←  اوامــر الحماية & القفل
...
🎖┊لـ تقييـم اداء البـوت ارسـل ← تقييم""",
    ...
    if m.data == f"commands10:{m.from_user.id}":
        m.edit_message_text(
            f"""⦇ ⑩ ⦈  ←  اوامــر الذكــاء الاصطنــاعـي 🧙

-  الذكــاء الاصطنــاعــي 🦾  
ٴ⋆┄─┄─┄─┄─┄─┄─┄┄⋆
اول بوت حماية يدعم الذكاء الاصطناعي Groq 🔅
...
بوت + سؤالك""",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("رجوع ↩️", callback_data=f"commandsback:{m.from_user.id}")]]
            ),
        )
...
🎖┊لـ تقييـم اداء البـوت ارسـل ← تقييم""",
            reply_markup=InlineKeyboardMarkup(
                [
                    ...
                    [
                        InlineKeyboardButton("𝟭𝟬", callback_data=f"commands10:{m.from_user.id}"),
                    ]
                ]
            ),
        )
...
⌯ زوجني ↣ لجلب زوجه او زوج
⌯ طلاق ↣ لطلاق الزوجه او الزوج
⌯ ممتلكاتي ↣ لجلب ممتلكاتك بالبوت
⌯ رفع ↣ ↢ تنزيل طفل
⌯ رفع ↣ ↢ تنزيل كلب
⌯ رفع ↣ ↢ تنزيل خدامه
⌯ رفع ↣ ↢ تنزيل بنتي
...
⌯ رفع ↣ ↢ تنزيل مرتي
⌯ رفع ↣ ↢ تنزيل زوجي
⌯ مسح ↣ بالرد لمسح الرتبه من الشخص
""",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
...
⌯ تفعيل ↣ لتفعيل البوت بالقروب
⌯ تعطيل ↣ لتعطيل البوت بالقروب
⌯ المغادرة ↣ لمغادرة البوت من القروب
⌯ الاحصائيات ↣ لجلب احصائيات البوت
⌯ اذاعه ↣ بالرد لنشر الرسالة للقروبات
⌯ اذاعه خاص ↣ بالرد لنشر الرسالة للاشخاص
⌯ المشتركين ↣ لجلب عدد مشتركين البوت
⌯ القروبات ↣ لجلب عدد قروبات البوت

⌯ رفع Dev ↣ لرفع مطور ثانوي
⌯ تنزيل Dev ↣ لتنزيل مطور ثانوي
⌯ رفع Myth ↣ لرفع مطور
⌯ تنزيل Myth ↣ لتنزيل مطور
⌯ رفع M ↣ لرفع مطور اساسي
⌯ تنزيل M ↣ لتنزيل مطور اساسي
""",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
    ...
    if m.data == f"back_to_commands:{m.from_user.id}":
        if admin_pls(m.from_user.id, m.message.chat.id):
            m.edit_message_text(
                """⎙┊يـوجـد ←  𝟏𝟎 قوائـم فـي البــوت 🎖
ٴ⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆
 ⦇ ① ⦈  ←  اوامــر الحماية & القفل
