...
def funFunc(c,m,k,channel):
   ...
   ################# WORDS #################
   if text.startswith('اضافه ك '):
     word = text.split(None, 2)[2]
     r.sadd(f'{Dev_Zaid}:UserWords:{m.from_user.id}', word)
     return m.reply(f'{k} ابشر ضفت الكلمة: {word}')

   if text == 'كلمات':
     words = r.smembers(f'{Dev_Zaid}:UserWords:{m.from_user.id}')
     if not words:
       return m.reply(f'{k} ما عندك كلمات مضافة')
     else:
       txt = '⇜ كلماتك المضافة:\n'
   ...
   if text in ['مين في الكول', 'من فالكول', 'الي في الكول']:
       def voice_chat_list_sync():
           try:
               ...
               async def get_list():
                   chat_full = await c.get_chat(m.chat.id)
                   if not chat_full.voice_chat:
                       return None
                       
                   try:
                       group_call = chat_full.voice_chat
                       input_call = InputGroupCall(id=group_call.id, access_hash=group_call.access_hash)
                       
                       participants_data = await c.invoke(
                       ...
                       for p in participants_data.participants:
                           try:
                               user = await c.get_users(p.peer.user_id)
                               users_list.append(f"• {user.mention} (`{user.id}`)")
                               count += 1
                           except:
                               continue
                       ...
                       return f"⇜ عدد الموجودين في الكول: {count}\n\n" + "\n".join(users_list)
                   except Exception as raw_err:
                       print(f"Raw API Error: {raw_err}")
                       return f"{k} معليش، التليجرام مانع البوتات العادية تشوف مين في الكول حالياً.\n\n⇜ الكول شغال ✅"
               ...
               if res:
                   ...
                   m.reply(f"{k} مفيش حد في الكول حالياً")
                   
           except Exception as e:
               print(f"Voice Chat Error: {e}")
               m.reply(f"{k} مفيش حد في الكول أو الكول مقفول")

       Thread(target=voice_chat_list_sync).start()
       return
   ################# WORDS #################
   ...
   if text == 'تقييم' or text == 'التقييم':
       if r.get(f'{Dev_Zaid}:rated:{m.from_user.id}'):
           return m.reply(f'{k} عفواً عزيزي، لقد قمت بالتقييم مسبقاً!')
           
       bot_name = r.get(f'{Dev_Zaid}botname') if r.get(f'{Dev_Zaid}botname') else 'شهد'
       ...
       rating_text = f"**- مرحبـاً بك عـزيـزي {user_mention} 🫂**\n**- فـي قائمـة تقييـم البـوت ⇜ ⌠ {bot_name} ⌡ 🎖**\n**- قم بـ تقييـم اداء البـوت**\n**- لـ دعمنا وتحفيزنـا لـ تقديـم المزيـد والافضـل 🏆**"
       ...
       return m.reply(rating_text, reply_markup=keyboard)
