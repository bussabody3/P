...
from youtube_search import YoutubeSearch as Tepthon
...
@Client.on_message(filters.text, group=32)
def ytdownloaderHandler(c,m):
    k = r.get(f'{Dev_Zaid}:botkey')
    channel = r.get(f'{Dev_Zaid}:BotChannel') if r.get(f'{Dev_Zaid}:BotChannel') else 'A7718BOT'
    Thread(target=yt_func,args=(c,m,k,channel)).start()
    
def yt_func(c,m,k,channel):
   if m.chat.type != ChatType.PRIVATE and not r.get(f'{m.chat.id}:enable:{Dev_Zaid}'):
        return False 
   ...
   if text.startswith('ابحث '):
     ...
     try:
         query = text.split(None,1)[1]
         results=Tepthon(query,max_results=8).to_dict()
         res_text = f"**🔍 نتائج البحث عن: {query}**\n\n"
         for res in results:
           title = res['title']
           url = f"https://www.youtube.com/watch?v={res['id']}"
           res_text += f"• [{title}]({url})\n\n"
         m.reply(res_text, disable_web_page_preview=True)
   ...
   if text.startswith('yt '):
     ...
     if r.get(f':disableYT:{Dev_Zaid}'):  return
     query = text.split(None,1)[1]
     results=Tepthon(query,max_results=1).to_dict()
     res = results[0]
     title = res['title']
